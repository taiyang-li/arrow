...
/*
// Private and native implementation 
class HadoopFileSystem::HadoopFileSystemNativeImpl {
  public:
  HadoopFileSystemNativeImpl(): port_(0) {}
  ...
  Status Connect(const HdfsConnectionConfig* config) {
      ...
      for (const auto& kv : config->extra_conf) {
        int ret =
            hdfsBuilderConfSetStr(builder, kv.first.c_str(), kv.second.c_str());
  ...
  Status GetWorkingDirectory(std::string* out) {
    char buffer[2048];
    if (hdfsGetWorkingDirectory(fs_, buffer, sizeof(buffer) - 1) == nullptr) {
  ...
  Status OpenWritable(const std::string& path, bool append, int32_t buffer_size,
    ...
    errno = 0;
    hdfsFile handle =
        hdfsOpenFile(fs_, path.c_str(), flags, buffer_size, replication,
  ...
  Status Copy(const std::string& src, const std::string& dst) {
    int ret = hdfsCopy(fs_, src.c_str(), fs_, dst.c_str());
    CHECK_FAILURE(ret, "Rename");
    return Status::OK();
  }

  Status Move(const std::string& src, const std::string& dst) {
    int ret = hdfsMove(fs_, src.c_str(), fs_, dst.c_str());
    CHECK_FAILURE(ret, "Rename");
    return Status::OK();
  }

  Status Chmod(const std::string& path, int mode) {
    int ret = hdfsChmod(fs_, path.c_str(), static_cast<short>(mode));  // NOLINT
    CHECK_FAILURE(ret, "Chmod");
    return Status::OK();
  }

  Status Chown(const std::string& path, const char* owner, const char* group) {
    int ret = hdfsChown(fs_, path.c_str(), owner, group);
